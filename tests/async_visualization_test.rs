...
async fn async_task(id: u32) -> u32 {
    ...
    let _guard = ExitGuard { fn_name: "async_task", task_id };
...
async fn test_async_task_visualization() {
    ...
    assert!(tasks.len() >= 5, "Expected at least 5 tasks, found {}", tasks.len());
    ...
    assert!(completed_count >= 5, "Expected at least 5 completed tasks, found {}", completed_count);
