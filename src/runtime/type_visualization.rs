...
use super::variables::{VariableValue, Variable};
...
impl TypeVisualizer {
    ...
    {
        self.visualizers.insert(type_name.to_string(), Box::new(visualizer));
    }
    ...
    fn register_default_visualizers(&mut self) {
        ...
        self.register_visualizer("Vec", |var| {
            if let VariableValue::Vec { elements, length, capacity } = &var.value {
        ...
        self.register_visualizer("HashMap", |var| {
            if let VariableValue::HashMap { entries, size, capacity } = &var.value {
    ...
    pub fn create_composite_visualization(&self, type_name: &str, var: &Variable, depth: usize) -> String {
        ...
        match &var.value {
            ...
            VariableValue::Complex { summary, fields, children, .. } => {
